...
def apply_liger_kernel_to_qwen2_t6(
    rope: bool = True,
    ...
    from model import modeling_qwen2_t6

    from model.modeling_qwen2_t6 import Qwen2Model



    if rope:
        modeling_qwen2_t6.apply_rotary_pos_emb = liger_rotary_pos_emb
    if rms_norm:
        modeling_qwen2_t6.Qwen2RMSNorm = LigerRMSNorm
    ...
    if cross_entropy:
        if transformer_version >= version.parse(SUPPORTED_TRANSFORMER_VERSION):
            ...
            modeling_qwen2_t6.CrossEntropyLoss = LigerCrossEntropyLoss

    if fused_linear_cross_entropy:
        if transformer_version >= version.parse(SUPPORTED_TRANSFORMER_VERSION):
            modeling_qwen2_t6.Qwen2ForCausalLM.forward = qwen2_lce_forward
        else:  # if version < 4.46.1
            logger.warning(TRANSFORMER_DEPRECATION_WARNING)
            modeling_qwen2_t6.Qwen2ForCausalLM.forward = qwen2_lce_forward_deprecated

    if swiglu:
        modeling_qwen2_t6.Qwen2MLP = LigerSwiGLUMLP
...
def main():
    ...
    apply_liger_kernel_to_qwen2_t6(model=model)
    ...
    # def group_texts_2(examples, block_size=2048):
    #     """
    #     Concatenate all tokenized samples in the current batch,
    #     chunk them to exactly `block_size`, and drop leftover tokens.
    #     """
    #     # 1) Flatten out the lists for each key across the batch
    #     concatenated = {k: sum(examples[k], []) for k in examples.keys()}
    #     total_length = len(concatenated["input_ids"])

    #     # 2) If the batch doesn't have enough tokens to form one full chunk, skip
    #     #    by returning empty arrays (HF Datasets will drop these)
    #     if total_length < block_size:
    #         return {k: [] for k in examples.keys()}  # effectively drops this batch

    #     # 3) Round `total_length` down to the nearest multiple of `block_size`
    #     total_length = (total_length // block_size) * block_size

    #     # 4) Slice into chunks of `block_size`
    #     result = {
    #         k: [
    ...
    #     # 5) For causal LM, "labels" are just the same as "input_ids"
    #     result["labels"] = result["input_ids"].copy()
    #     return result
